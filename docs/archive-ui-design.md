...
4. **Delete intentionally:** use the existing accessible confirmation dialog rather than browser confirm. Explain that deletion removes the session across devices and clears its Malink messages. Lead with the conditional recovery path: if the Agent retains history, the conversation-list overflow → Agent History can rebuild a new session and restore history, at greater cost than restoring an archive. Mention scratch files only for scratch sessions. The confirmation suggests archive when the intent is merely to tidy the list.
...
- Delete recovery copy follow-up: the existing archive browser workflow passes. The production delete dialog was additionally checked at 320×568, 360×844, 390×844, 768×844 and 1280×844: recovery conditions are visible, both footer actions remain inside the viewport, there is no horizontal overflow, and cancelling deletion allows restoring the archived conversation. The 320px and desktop screenshots were visually inspected. This remains browser simulation, not physical APK or live history-recovery verification.
