...
- **任何涉及 UI 的功能改动，都必须从用户场景和合理的使用流程出发决策设计，不能以“功能已添加”作为完成标准。** 实现前必须明确用户是谁、在什么场景下要完成什么任务，并梳理发现入口、理解选项、执行操作、确认结果以及撤销或恢复的完整流程；据此决定信息层级、默认状态、入口位置和交互方式。
  - 必须同时考虑 PWA 桌面端与 Android APK 小屏端的实际使用，评估屏幕空间、操作频率和触达成本；优先融入已有交互，不能为每个新增功能机械地常驻增加一整栏或一组按钮，挤占主要内容。
  - 用户应能理解当前视图、可选操作及其后果；命名、选中态、反馈和恢复入口必须清晰。必须考虑首次使用、空状态、已有大量数据、历史数据迁移和跨设备同步，不能把难以解释的内部状态直接暴露给用户。
  - 完成前必须按上述用户流程验证 UI，特别检查 APK 小屏下的可发现性、可理解性和空间占用；交付时说明设计对应的用户场景及验证情况，不能仅以接口打通、按钮可点击或自动化测试通过作为 UI 验收依据。
