...
export function malinkAgentInstructions(toolsAvailable: boolean): string {
    return [
        ...
        toolsAvailable
            ? 'Malink MCP tools are attached. Use get_malink_context for environment and session details. Use send_file with an absolute local path to deliver files to this conversation: type=image for an image preview, type=document for a downloadable file, type=markdown or type=code for rendered text. A queued result is not confirmed delivery; report failures accurately. For user-selected @conversation references, call read_conversation_reference with the supplied referenceId and paginate with nextOffset; this reads a snapshot without contacting the source agent.'
