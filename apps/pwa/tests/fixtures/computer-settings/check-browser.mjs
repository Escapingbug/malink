...
try {
 ...
 for (const width of [390, 1280]) {
  ...
  await page.getByRole('button', { name: 'Manage computer', exact: true }).waitFor();
  await page.screenshot({ path: `/tmp/malink-settings-qa/list-${width}.png` });
  await page.getByRole('button', { name: 'Manage computer', exact: true }).click();
  ...
  await page.getByRole('button', { name: 'Start update session', exact: true }).click();
